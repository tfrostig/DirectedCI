...
ggplot(results_df, aes(y = reorder(endpoint, HR), x = HR)) +
  ...
  geom_errorbarh(aes(xmin = std_lower, xmax = std_upper),
                 height = 0.3, linewidth = 1, color = "#E41A1C") +
  geom_point(aes(shape = significant), size = 3.5, color = "#E41A1C") +
...
This plot compares all available CI methods for the significant endpoints:
...
| BY Unadjusted | Multiplicity | Controls family-wise error across endpoints |
| BY DP-/MP | Multiplicity + Direction | Asymmetric CI favoring expected benefit |
