...
All CI functions work with a single observation `y` and its standard deviation `sd`. They return a 4-element vector `c(ll, ul, lr, ur)` where positions 3 and 4 contain the lower and upper CI bounds (positions 1-2 are used for split intervals).
...
cat("Shortest Marginal CI:", round(shortest_marg[3:4], 3), "\n")
...
cat("MP+ Marginal CI:     ", round(mp_marg[3:4], 3), "\n")
```
...
cat("Shortest Conditional CI:", round(shortest_cond[3:4], 3), "\n")
...
cat("DP+ Conditional CI:     ", round(dp_cond[3:4], 3), "\n")

# 3. Modified Pratt Conditional CI
mp_cond <- modified_pratt_conditional_ci(
  y = y_sig, sd = sd_sig, ct = ct, r = 1.3, alpha = alpha, direction = "positive"
)
cat("MP+ Conditional CI:     ", round(mp_cond[3:4], 3), "\n")
```
...
comparison <- data.frame(
  ...
  Lower = c(shortest_marg[3], mp_marg[3],
            shortest_cond[3], dp_cond[3], mp_cond[3]),
  Upper = c(shortest_marg[4], mp_marg[4],
            shortest_cond[4], dp_cond[4], mp_cond[4])
)
...
for (i in seq_along(ordered_means)) {
  ...
  # CI is returned as c(ll, ul, lr, ur); single interval uses positions 3 and 4
  lower_marginal[i] <- marginal_ci[3] * ordered_ses[i]
  upper_marginal[i] <- marginal_ci[4] * ordered_ses[i]
}
...
for (i in seq_along(ordered_means)) {
  ...
  # CI is returned as c(ll, ul, lr, ur); single interval uses positions 3 and 4
  lower_conditional[i] <- cond_ci[3] * ordered_ses[i]
  upper_conditional[i] <- cond_ci[4] * ordered_ses[i]
}
